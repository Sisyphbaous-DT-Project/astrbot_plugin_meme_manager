...
class MemeSender(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        self.send_meme_when_empty = self.config.get("send_meme_when_empty", True)
        self.always_send_meme_when_empty = self.config.get(
            "always_send_meme_when_empty", True
        )
    ...
    async def resp(self, event: AstrMessageEvent, response: LLMResponse):
        ...
        event.set_extra(
            ...
            bool(self.found_emotions) and not response.completion_text,
        )
    ...
    async def _send_memes_streaming(
        self, event: AstrMessageEvent, force_send: bool = False
    ):
        ...
        try:
            if not force_send:
                ...
                logger.debug(
                    "[meme_manager] 流式模式下检测到纯表情回复，跳过表情出现概率判断"
                )
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        ...
        if result.result_content_type == ResultContentType.STREAMING_FINISH:
            if self.streaming_compatibility:
                empty_meme_reply = event.get_extra("meme_manager_empty_meme_reply")

                if empty_meme_reply and not self.send_meme_when_empty:
                    logger.debug(
                        "[meme_manager] 流式模式下检测到纯表情回复，但配置禁止发送"
                    )
                    self.found_emotions = []
                    return
                ...
                force_send = (
                    empty_meme_reply
                    and self.send_meme_when_empty
                    and self.always_send_meme_when_empty
                )

                await self._send_memes_streaming(event, force_send=force_send)
            return
        ...
        try:
            ...
            if self.found_emotions:
                is_empty_meme_reply = not cleaned_components

                if is_empty_meme_reply and not self.send_meme_when_empty:
                    logger.debug("[meme_manager] 检测到纯表情回复，但配置禁止空回复发送表情包")
                else:
                    force_send_for_empty_reply = (
                        is_empty_meme_reply
                        and self.always_send_meme_when_empty
                    )
                    ...
                    should_send_meme = force_send_for_empty_reply or random_value <= threshold

                    if force_send_for_empty_reply:
                        logger.debug(
                            "[meme_manager] 检测到纯表情回复，已开启必定发送，跳过表情出现概率判断"
                        )
                    else:
                        logger.debug(
                            f"[meme_manager] 表情发送概率判断: random={random_value}, threshold={threshold}, send={should_send_meme}"
                        )
                    ...
                    if should_send_meme:
                        ...
                        if emotion_images:
                            ...
                            if not cleaned_components:
                                if self.send_meme_when_empty:
                                    logger.debug("[meme_manager] 检测到纯表情回复，直接发送图片消息链")
                                    ...
                                    for temp_file in temp_files:
                                        try:
                                            if os.path.exists(temp_file):
                                                os.remove(temp_file)
                                                logger.debug(f"[meme_manager] 已清理未发送的临时文件: {temp_file}")
                                        except Exception as e:
                                            logger.error(f"[meme_manager] 清理未发送的临时文件失败: {e}")
                                    event.set_extra("meme_manager_temp_files", None)

                            elif use_mixed_message:
